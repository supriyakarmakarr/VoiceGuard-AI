...
(()=>{
 ...
 let visible=true,active=false,signal=null,rotation=0,last=0,frame=0,width=1,height=1,px=0,py=0,tx=0,ty=0;
 ...
 function line(points3,r,alpha=.2,lineWidth=.7){ctx.beginPath();points3.forEach((p,i)=>{const q=project(...p,r);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);});ctx.strokeStyle=`rgba(77,119,172,${alpha})`;ctx.lineWidth=lineWidth;ctx.stroke();}
 function draw(now,once=false){if(!once&&(!visible||document.hidden))return;ctx.clearRect(0,0,width,height);const r=Math.min(width*.30,height*.33);px+=(tx-px)*.045;py+=(ty-py)*.045;
  ...
  const sorted=points.map(p=>project(...p,r)).sort((a,b)=>a.z-b.z);for(const p of sorted){ctx.fillStyle=`rgba(69,112,169,${.16+(p.z+1)*.2})`;ctx.beginPath();ctx.arc(p.x,p.y,.65+(p.z+1)*.5,0,Math.PI*2);ctx.fill();}
  // Idle signal is illustrative. Analysis and microphone modes replace it with measured values.
  ctx.lineWidth=1.15;for(let echo=-1;echo<=1;echo++){ctx.beginPath();for(let i=0;i<=110;i++){const x=(i/110-.5)*1.65,envelope=Math.exp(-x*x*4);let value=signal?.length?signal[Math.min(signal.length-1,Math.floor(i/111*signal.length))]:Math.sin(i*.46+rotation*3)*Math.cos(i*.115)*.24;value*=signal? .55:1;const p=project(x,value*envelope+echo*.045,0,r);i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);}ctx.strokeStyle=echo===0?'rgba(61,110,175,.75)':'rgba(103,150,207,.22)';ctx.stroke();}
 ...
 function loop(now){frame=0;if(!visible||document.hidden)return;if(now-last>(weak?60:40)){if(!reduce.matches)rotation+=active?.009:.003;draw(now);last=now;}if(!reduce.matches||active)frame=requestAnimationFrame(loop);}
 function start(){if(!frame&&visible&&!document.hidden)frame=requestAnimationFrame(loop);}
 figure.addEventListener('pointermove',e=>{if(reduce.matches)return;const b=figure.getBoundingClientRect();tx=((e.clientX-b.left)/b.width-.5)*2;ty=((e.clientY-b.top)/b.height-.5)*2;start();},{passive:true});figure.addEventListener('pointerleave',()=>{tx=ty=0;});
